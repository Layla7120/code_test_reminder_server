...
이전 문서(현 [`docs/마이그레이션-기록.md`](../docs/마이그레이션-기록.md))의 Flask vs Spring 비교표는
변수가 최소 네 개(커넥션 풀 15 vs 20, 캐시 커버리지, 시나리오 구성, 처리량) 섞여 있어
**어느 변수가 차이를 만들었는지 말할 수 없었다.**
게다가 그 측정은 랭킹 점수 이중 계상 버그가 살아있던 상태에서 잰 것이라 숫자 자체를 쓸 수 없다.
